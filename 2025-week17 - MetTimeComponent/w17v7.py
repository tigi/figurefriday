...
select_mode = html.Div([
   ...
   ], style={ 'display': 'inline-block', 'verticalAlign': 'top', 'marginTop': '30px'})
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col( html.H1("Migration Flows 2024"),),
        dbc.Col(   )
...
def update_map(selected_country, selected_mode):
    ...
    for _, row in dff.iterrows():
        ...
        people = row['2024']
        radius = 5 + math.sqrt(people) * 0.01
        ...
        arrows_and_markers.append(
            dl.CircleMarker(
                ...
                fill=True,
                fillOpacity=0.7,
                children=[dl.Tooltip(f"{target_country}: {people:,} people")]
            )
        )
    ...
    return arrows_and_markers, create_country_info_card(selected_country, selected_mode,netto_migration, dff), f"hoi {selected_mode}"
